...
3. **建立題目資料夾與檔案**：`LeetCodePractice/Problems/<ProblemName>/<ProblemName>.cs`
   - 使用 `ProblemTemplate.cs` 的模式
   - 將類別名稱取代為使用者的 ProblemName
   - 加入 TODO 註解供題目描述與實作使用
   - **不要**實作解法

4. **建立測試資料夾與檔案**：`LeetCodePractice.Tests/<ProblemName>/<ProblemName>Tests.cs`
   - 使用 `ProblemTemplateTests.cs` 的模式
   - 加入至少一個範例測試案例
   - 包含佔位符註解，提示更新預期值
   - **不要**實作含有正確值的測試驗證

5. **更新題目清單**：將新題目（含分類）加入 `.agents/skills/leetcode-helper/resources/problem-list.md`

6. **確認**：向使用者顯示建立的資料夾路徑，並提醒他們：
   - 加入題目描述作為註解
   - 加入含有預期值的測試案例
   - 實作他們的解法
...
**動作**：
- 與使用者確認
- 刪除 `LeetCodePractice/Problems/WaferScanOptimizer/` 資料夾
- 刪除 `LeetCodePractice.Tests/WaferScanOptimizer/` 資料夾
- 更新 `.agents/skills/leetcode-helper/resources/problem-list.md`
- 執行 `dotnet test`
- 輸出：「✓ 已移除 WaferScanOptimizer 及其測試資料夾。dotnet test 通過。」
...
1. **重新命名資料夾**：
   - 將 `LeetCodePractice/Problems/<OldName>/` 重新命名為 `LeetCodePractice/Problems/<NewName>/`
   - 將 `LeetCodePractice.Tests/<OldName>/` 重新命名為 `LeetCodePractice.Tests/<NewName>/`

2. **更新類別名稱**：讀取兩個 `.cs` 檔案，將類別宣告從 `<OldName>` 更新為 `<NewName>`。
...
**動作**：
- 將 `LeetCodePractice/Problems/TwoSum/` 重新命名為 `LeetCodePractice/Problems/TwoSumV2/`
- 將 `LeetCodePractice.Tests/TwoSum/` 重新命名為 `LeetCodePractice.Tests/TwoSumV2/`
- 更新兩個 `.cs` 檔案內部的類別名稱
- 更新 `.agents/skills/leetcode-helper/resources/problem-list.md`
- 執行 `dotnet test`
...
2. **掃描**：掃描 `LeetCodePractice/Problems/` 中所有子資料夾，每個子資料夾代表一個題目。與題目清單交叉比對。

3. **檢查測試狀態**：對於每個題目，檢查 `LeetCodePractice.Tests/<ProblemName>/<ProblemName>Tests.cs` 是否存在。
...
5. **產生 `LeetCodeProblems.md`**：在儲存庫根目錄建立或覆寫 `LeetCodeProblems.md`，格式如下：
   ...
   **LC xxx Original**
   簡短描述...
   - [實作檔](LeetCodePractice/Problems/ProblemName/ProblemName.cs)
   - [測試檔](LeetCodePractice.Tests/ProblemName/ProblemNameTests.cs)
   ```
